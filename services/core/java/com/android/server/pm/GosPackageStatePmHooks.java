...
class GosPackageStatePmHooks {
    ...
    static void serialize(PackageUserState packageUserState, TypedXmlSerializer serializer) throws IOException {
        GosPackageStatePm ps = packageUserState.gosPackageState;
        if (ps == null) {
            return;
        }
    ...
    // PackageManagerService#getGosPackageState
    @Nullable
    static GosPackageState get(PackageManagerService pm, String packageName, int userId) {
        ...
        synchronized (pm.mLock) {
            ...
            return getInner(ps, userId, selfCheck);
        }
    }

    @GuardedBy("PackageManagerService.mLock")
    @Nullable
    private static GosPackageState getInner(PackageSetting ps, int userId, boolean selfCheck) {
        GosPackageStatePm gosPs = ps.getGosPackageState(userId);
        if (gosPs == null) {
            gosPs = GosPackageStatePmHooks.maybeGetForSharedUserPackage(ps, userId);
        }
        ...
        int derivedFlags = GosPackageStatePmHooks.maybeDeriveFlags(gosPs, ps);
    ...
    @Nullable
    // PackageManagerService#setGosPackageState
    static GosPackageState set(PackageManagerService pm, String packageName,
        ...
        synchronized (pm.mLock) {
            ...
            SharedUserSetting sharedUser = ps.getSharedUser();
            if (sharedUser != null) {
            ...
            pm.scheduleWritePackageRestrictionsLocked(userId);

            result = getInner(ps, userId, false);
        }
    ...
    @GuardedBy("PackageManagerService.mLock")
    private static int maybeDeriveFlags(final GosPackageStatePm ps, PackageSetting pkgSetting) {
        if ((ps.flags & FLAG_STORAGE_SCOPES_ENABLED) == 0) {
        ...
        SharedUserSetting sharedUser = pkgSetting.getSharedUser();
    ...
    @GuardedBy("PackageManagerService.mLock")
    // see GosPackageStatePm doc
    private static GosPackageStatePm maybeGetForSharedUserPackage(PackageSetting packageSetting, int userId) {
        SharedUserSetting sharedUser = packageSetting.getSharedUser();
        if (sharedUser == null) {
            return null;
        }

        ArraySet<PackageSetting> packages = sharedUser.packages;
        if (packages == null) {
            return null;
        }

        for (int i = 0, m = packages.size(); i < m; ++i) {
            PackageSetting setting = packages.valueAtUnchecked(i);
            GosPackageStatePm ps = setting.getGosPackageState(userId);
            if (ps != null) {
                return ps;
            }
        }
